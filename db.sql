...
CREATE TABLE user (
  id BIGINT NOT NULL AUTO_INCREMENT COMMENT 'Primary key',
  open_id VARCHAR(255) NOT NULL DEFAULT '' COMMENT 'OpenID，用户唯一标识',
...
CREATE TABLE conversation (
  id BIGINT NOT NULL AUTO_INCREMENT COMMENT 'Primary key',
  uuid VARCHAR(36) NOT NULL DEFAULT '' COMMENT '会话 UUID',
  user_id BIGINT NOT NULL DEFAULT '-1' COMMENT '用户 ID',
  history_json JSON NOT NULL DEFAULT '[]' COMMENT '会话历史（JSON 格式）',
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
